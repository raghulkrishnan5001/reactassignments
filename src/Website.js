...
export class Website extends Component {
  ...
  handleclear=(e)=>{e.preventDefault();this.setState({clear:true})}
  handletoggle=(e)=>{e.preventDefault();this.setState({clear:!this.state.clear})}
  handleDelete=(i)=>{
    let updatedArray=[...this.state.datas]
...
datas:updatedArray   
  ...
  render() {
    return (
      <div className='a'>
        <h1> OUR TOURS</h1>
        {this.state.datas.map((data,index) => <div key={data.id}><div className='b'><img src={data.image} />
          <div className='c'>
          ...
          <div>
            <p>{this.state.clear?data.info:data.info.slice(0,150)}
            <a href="#"className='g' onClick={this.handletoggle}>{this.state.clear?'Read Less':'Read More'}</a>
            </p>
                            
          </div>
          <center><button className='btn btn-outline-danger m-4' onClick={()=>this.handleDelete(index)}> NOT INTERESTED</button></center>
